...
CIDR ranges, port ranges, and multiple applicable block policies are currently
reported as `network enforcement unsupported`. The command stops before hook
attachment instead of silently running such a policy as though it were
enforced. Observe/audit policies continue to use post-event matching.
...
`enforced: true`. Ring-buffer pressure can lose evidence but does not turn a
computed denial into an allow.
